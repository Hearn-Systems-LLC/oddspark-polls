...
- Voter Code generation and management for invite-only Polls. Creators with
  ...
  inventory with redemption status, and copy all codes to clipboard.
- Invite-code-gated voting. When Voter Codes are enabled, each voter must enter
  a valid unused code to submit their ballot. Codes are consumed atomically with
  the Vote in a single D1 transaction, so concurrent submissions on the same code
  produce exactly one accepted Vote. The trust badge shows INVITE CODE REQUIRED
  when admission is active.
