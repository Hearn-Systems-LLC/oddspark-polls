...
- [x] [Review][Patch] `invalid_selection` discards the strategy's message and `fieldErrors`, showing retry copy for a ballot that can never succeed [src/modules/voting/index.ts:291-293] — fixed: strategy message + `fieldErrors`/`reasonCodes` preserved on both rejection codes; unit-pinned
