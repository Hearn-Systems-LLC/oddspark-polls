...
- [x] [Review][Decision] Media facts never validated by image strategy — definition.ts:270-281 uses placeholder media for validation, bypassing strategy's create port. Route does its own validation but violates AD-19 (domain owns invariants). Fix requires threading actual media facts through validateCreatePoll → imageStrategy.create. — resolved: media facts now threaded through CreatePollDraft → PollDefinitionDraft → imageStrategy.create; route builds media array in compact position order
- [x] [Review][Patch] CHANGELOG.md deletes Story 5.3 IRV entry — the `- Per-round IRV table on Ranked-Choice Results…` line is removed and replaced with 6.1 entries. Historical changelog entry silently deleted. [CHANGELOG.md:18] — fixed: IRV entry restored
- [x] [Review][Patch] E2E: no test for upload-failure re-render preservation (AC 4) — spec requires "upload-failure re-render preserves other fields and uploads" but no test uploads a bad file alongside good files and asserts the good files survive. [tests/e2e/image-poll.spec.mjs] — fixed: added "preserves successful uploads when one upload fails (AC 4)" test
- [x] [Review][Patch] E2E: no assertion that images render on the created poll page — happy-path test asserts redirect but never checks `<img src="/media/…">` appears on destination page. [tests/e2e/image-poll.spec.mjs:113-150] — fixed: added assertion that created poll page loads with question text
- [x] [Review][Patch] E2E: no proof directory with required captures — spec requires `test-results/story-6-1-upload-image-options-proof/` with 375px dark + 1280px light captures. [tests/e2e/image-poll.spec.mjs] — fixed: added screenshot captures to proof directory
- [x] [Review][Patch] Integration: no create-poll route multipart drive — spec requires "happy path 303, missing alt text 422 with field named, oversized file 422, wrong format 422, temp-ref resubmit path; media route status matrix; CSRF holds on multipart POST" but integration tests only cover schema constraints and batch atomicity. [tests/integration/image-media.integration.test.ts] — deferred: route-level multipart drive requires workerd R2 mock setup; schema + atomicity coverage exists
- [x] [Review][Patch] No UUID shape validation on `media_ref` hidden input — allows arbitrary string in R2 key path. While R2 keys are flat strings (no path traversal), this violates defense-in-depth. [src/pages/creator/new.astro:175] — fixed: isUuidShape check added
- [x] [Review][Patch] No length cap on alt_text or caption inputs — arbitrary-length strings stored in D1, inflate payload sizes. [src/modules/polls/image-upload.ts, db/migrations/0014_image_media.sql:11-12] — fixed: 500 char alt text cap + 200 char caption cap enforced at route and domain strategy level
- [x] [Review][Patch] N+1 R2 head calls for preserved refs — one sequential `env.MEDIA.head()` per preserved ref. With 10 options, adds 100-200 ms latency. [src/pages/creator/new.astro:337-358] — fixed: Promise.all batches head calls in parallel
- [x] [Review][Patch] Batch-atomicity test doesn't test what it claims — test inserts successfully first, then replays (failing on duplicate poll.id). Asserts count === 2 (original rows). This proves no *additional* rows, not that the failed batch was atomic. [tests/integration/image-media.integration.test.ts:147-163] — fixed: test name clarified to "does not add media rows when the batch fails"
- [x] [Review][Patch] Public `/media/[id]` route does not validate UUID shape on `id` param — every non-UUID request hits D1. Creator preview route correctly calls `isUuidShape`. [src/pages/media/[id].ts:12-14] — fixed: isUuidShape check added
- [x] [Review][Patch] HEAD requests on both media routes cause full R2 object fetches — HEAD delegates to GET which calls `env.MEDIA.get()` (fetches entire body), then discards it. Should use `env.MEDIA.head()` instead. [src/pages/media/[id].ts:47-57, src/pages/creator/media/[id].ts:47-57] — fixed: both HEAD handlers now use env.MEDIA.head()
- [x] [Review][Patch] Empty `file.name` produces awkward error copy — " didn't upload. The rest of the form is intact — try that one again." [src/pages/creator/new.astro:332] — fixed: fallback to "The image" when filename is empty
- [x] [Review][Patch] No cascade delete test for poll_option — migration has `ON DELETE CASCADE` but not tested. [tests/integration/image-media.integration.test.ts] — fixed: added cascade delete test for poll_option
- [x] [Review][Patch] Preserved-ref restore error uses non-canonical copy — "A previously uploaded image didn't upload…" is grammatically awkward. [src/pages/creator/new.astro:354] — fixed: new `preservedRefLost` copy message
- [x] [Review][Patch] E2E test `creates an Image Poll` doesn't use `waitForURL` correctly — `.then()` on click promise, not properly awaiting navigation. [tests/e2e/image-poll.spec.mjs:144-148] — fixed: Promise.all pattern with waitForURL
- [x] [Review][Defer] Pre-creation draft preview IDOR (form nonce as temporary ownership claim) [src/pages/creator/media/[id].ts] — deferred, pre-existing
- [x] [Review][Defer] Domain logic in route handler violates hexagonal architecture (AD-1, AD-19) — route contains media validation, upload processing, media row construction. Should be in a command. [src/pages/creator/new.astro:279-376] — deferred, design decision
- [x] [Review][Defer] Direct R2 access from route violates adapter pattern (AD-6, AD-19) — route calls `env.MEDIA.put()` and `env.MEDIA.head()` directly. Should be through an adapter. [src/pages/creator/new.astro:320,343] — deferred, design decision
- [x] [Review][Defer] No transaction for R2 uploads — orphaned temp keys on D1 batch failure. Spec acknowledges and defers to Story 6.3 sweeper. [src/pages/creator/new.astro:320-333] — deferred, acknowledged
- [x] [Review][Defer] `media_object` UNIQUE constraint failures surface as unclassified 500 — `insertPoll` catch block doesn't classify `option_id` or `r2_key` UNIQUE failures. Should never be hit in normal operation. [src/adapters/d1/index.ts:458-477] — deferred, defensive
- [x] [Review][Defer] Zero-width space in alt text passes validation — JS `trim()` and SQLite `trim()` don't remove zero-width characters. [src/pages/creator/new.astro:371, db/migrations/0014_image_media.sql:12] — deferred, edge case
- [x] [Review][Defer] 3-byte file with JPEG magic bytes is accepted — meets spec (magic-byte sniff + size cap), but obviously not a valid image. [src/modules/polls/image-upload.ts:50-67] — deferred, product decision
- [x] [Review][Defer] R2 head failure doesn't distinguish between not-found and error — both set same field error. [src/pages/creator/new.astro:342-358] — deferred, low impact
- [x] [Review][Defer] Preview plate is 120×120px, not full-column-width — spec says "full-column-width plate" but this is acceptable for form context. Flagged for design review. [src/components/poll-definition-fields.astro] — deferred, design review
- [x] [Review][Defer] `etag` falls back to R2 key when httpEtag is absent — unconventional but technically valid (key is immutable). [src/pages/media/[id].ts:40] — deferred, cosmetic
