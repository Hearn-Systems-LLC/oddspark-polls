...
UX-DR7: `trust-badge` — label-caps-lg line above the vote button mapping active Toggles to Voter-terms copy (`ONE VOTE PER BROWSER`, `ONE VOTE PER NETWORK`, `INVITE CODE REQUIRED`, `HUMAN CHECK ON SUBMIT`, `NO VPN OR DATACENTER CONNECTIONS`); absent entirely when all Toggles are off; stacks one item per line (never truncates or abbreviates); persists onto the Tally; no shield/lock iconography or "verified"/"secure" claims. The embedded editable Demo renders exactly one trust badge with the ballot above Turnstile and `VOTE`; its simultaneously visible Tally suppresses the duplicate. When the ballot becomes read-only, the canonical Tally owns the badge.
...
UX-DR16: `overlay` system — exactly four (delete-Poll confirm, delete-Comment confirm, Voter Code panel, Demo reset confirm), none stack; scrim + flat panel with top/bottom hairlines, no shadow or scale-in; focus trapped inside, `Esc` always closes, scrim-click dismisses the confirmations but not the code panel; focus returns to the invoking control on close; page behind does not scroll. Demo reset has the complete no-JavaScript server-open/cancel/POST baseline and pins title `RESET DEMO POLL?`, cancel `KEEP VOTES`, destructive action `RESET VOTES`, and enhanced pending label `RESETTING…`.
UX-DR17: Accessibility floor — every action keyboard-completable; focus order follows reading order; 2px/2px focus ring everywhere via the focus-ring token; on every post-submit render (Counted, already-voted, VPN-blocked, bad code, CAPTCHA fail, rate-limited, failed, and OAuth returns) the outcome line is `tabindex="-1"`, first in the main landmark, receives focus on load, and the document `<title>` leads with the outcome; Tally totals are one polite `aria-live` region announcing aggregate change, leader changes ("Pizza now leading, 47 percent"), `TIED`, and connection loss/restore — never per-bar chatter; option accessible names carry state and value; no icon-only controls anywhere; state never color alone; 48px tap targets on voting-surface controls, 44px elsewhere; no gestures required (rank builder is tap-to-assign, not drag). On `/`, an ordinary open render keeps the Demo after the build account, but every recoverable POST, accepted flash, returning duplicate, closed fallback, or operational failure places the complete Demo region first inside `<main>`; its focused outcome stays the first main content and leads `<title>` without duplicating the region.
...
UX-DR26: Landing page + Demo Poll — Newsreader opening statement, short technical build account, repository link, create entry, and Discover link; the Demo Poll renders inline as a complete votable Poll (same components, not a screenshot or reduced version) with all states handled inline. It is Multiple Choice with `Friday`, `Monday`, `Either works` in that order, and shows the authorized live Tally before a Vote as well as after one. Already-voted shows the live Tally; mid-reset shows the ordinary empty state; question is "Best day for a long weekend?" with no Deadline. Missing, malformed, unresolved, drifted, or initially unreadable Demo truth returns the private/no-store `503` state headed `DEMO UNAVAILABLE`; it never guesses another Poll. The readable no-script surface says `JavaScript is required for the human check on this Poll.` and never accepts a Vote without server-side challenge proof.
