...
**The `/discover` catalog reuses this row unchanged** — same title, same metadata line, same hairline rhythm, the whole row one tap target. Discover shows only open, Listed Polls, so every row carries the live indicator and none carries `CLOSED`. The catalog paginates newest-first with `NEWER` / `OLDER` text controls per `{components.pagination}` — label-caps entropy links at 48px targets, the exhausted end rendered `{colors.dim-dark}` and inert. Never infinite scroll, never a card grid.
...
Three exist and none of them stack: delete-Poll confirmation, delete-Comment confirmation, and the Voter Code list panel.
