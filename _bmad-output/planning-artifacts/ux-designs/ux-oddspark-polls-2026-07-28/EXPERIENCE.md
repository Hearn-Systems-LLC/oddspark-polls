...
|---|---|---|---|---|---|
| Landing page | `/` | Portfolio link, direct, search | What the platform is, how it was built, and the pinned Demo Poll (FR-25); a full-width `landing-footer` below the grid carries the create entry, Discover, the repository link, and the Hearn byline | UJ-5, UJ-6, UJ-7 | 1 |
...
| Creator — Poll detail | `/creator/polls/:pollId` | Poll list row | Monitor, edit description, close, delete, export, moderate Comments, reset Demo Poll (FR-4, 5, 22, 24, 26) | UJ-1 | 1 |
...
| Creator — Export | `/creator/polls/:pollId/export.csv`; `/creator/polls/:pollId/export.xlsx` | Poll detail | Owner-authorized raw Votes, Tally, and Summary download; XLSX is a direct synchronous download through 1,000 accepted Votes, with CSV as the larger-Poll path (FR-22) | UJ-1 | 1 |
...
| `comment` — Comment list | Tally view; Administrator exact-reference target | Complete newest-first plain text with `ANONYMOUS` when no name was supplied, no threading, reactions, truncation, or empty placeholder. On ordinary Results it is visible exactly where the Tally is visible; the owner alone sees one delete affordance per Comment. The Administrator sees a separate live-role-guarded list on the one resolved operator target, never owner controls on public Results (FR-24). |
...
| Export | Creator — Poll detail | Plain adjacent `EXPORT CSV` then `EXPORT XLSX` controls, each starting directly with no configuration dialog. Both remain 48px targets and share a row without narrow-screen overflow. XLSX downloads synchronously for Polls with at most 1,000 accepted Votes. For 1,001 or more, activation yields no attachment and the exact plain-text line `XLSX export supports up to 1,000 accepted votes. Download CSV for larger Polls.` with HTTP `409`; CSV remains available. No partial or truncated workbook is ever presented. Available on the creator surface only (FR-22). |
| `share-action` — Share action | Create confirmation, voting page, Tally view | A text-labelled `SHARE` control sitting beside the Poll's canonical URL, which is always rendered as visible, selectable text — the no-JavaScript baseline is "the URL is right there; copy it," and the surface is fully functional without JavaScript by construction (FR-28). With JavaScript, the control invokes the Web Share API (the device's native share sheet) where available and copies the link to the clipboard otherwise, confirming with a `{typography.label-caps}` `LINK COPIED` line beside the control that persists until the next interaction — not a toast. With JavaScript and clipboard capability, activating the URL text itself also copies and confirms `LINK COPIED`; the no-JavaScript baseline is unchanged. The confirmation posts one polite live-region announcement, in the same idiom as the codes-copied confirmation. The action never gates the Tally behind sharing and never renders vendor-specific social buttons. On the voting surface it is a `button-secondary`, never a competing primary action. |
| `public-repository-link` — Repository entry | Landing footer; canonical public voting and non-404 Results | One shared presentation seam renders the exact public repository destination. On Poll surfaces it follows Share in source and focus order, opens in the same tab, works without JavaScript, and uses a text label with a 44px minimum target (48px inside the `landing-footer`, matching its neighbours) and the standard focus outline. The Poll footer is absent from the embedded landing Demo, 404, creator, auth, administration, and moderation surfaces. |
| `landing-footer` — Landing footer | Landing page only, every variant including 503 | A `<footer>` landmark below `<main>`, after all main content in source and focus order, containing the byline and a `<nav aria-label="Landing">` with three pure server-rendered links — Create, Discover, repository — opening in the same tab with no JavaScript behavior. The byline sits outside the nav: `built by ` (trailing space) followed by the Hearn. wordmark SVG (`role="img"`, `aria-label="Hearn."`, so the link's accessible name is "built by Hearn."), one link to `https://hearn.systems` with `rel="noopener"` at a 44px minimum block target. The wordmark SVG is never `aria-hidden` — it is the attribution. No other surface renders the footer or the wordmark. |
...
| Administrator: target found | `/creator/moderation?target={canonical-reference}` | Escaped question as the focused target heading, then canonical voting link, `LIFECYCLE`, `DISCOVERY`, the Discovery action, and the complete newest-first Comment list with one `DELETE COMMENT` control per row. No owner, Visibility Setting, options, Tally, ballots, history, reason, or moderation enumeration. An empty list adds no placeholder. |
| Owner/Administrator: delete Comment | Authorized Results or exact operator target | `DELETE COMMENT` opens the shared confirmation, whose copy says the Vote and Tally remain. Confirming posts the fixed session-CSRF form and redirects to canonical fresh truth; Cancel, `Esc`, or the scrim closes and restores focus. Without JavaScript, a single native disclosure exposes the same real POST confirmation. |
| Comment deletion stale or failed | Authorized Results or exact operator target | Stale/missing targets disclose no protected fact. A failed atomic mutation leaves the Comment and representation version unchanged and returns safe reload-before-retry copy; no forgeable success query is shown. |
...
| Breakpoint | Behavior |
|---|---|
| below `{breakpoints.sm}` (default) | One column. Landing: statement, then build notes, then the Story 3.5 Demo Poll, then the `landing-footer` — byline on the first line, the three links stacked beneath at 48px targets. Voting: question, options, Comment, challenge, vote button, then Tally below. Availability grid becomes one row per slot with three targets per row. |
| `{breakpoints.sm}` to below `{breakpoints.lg}` | Same single column, wider margins, longer measure. No structural change. |
| `{breakpoints.lg}` and up | Two columns wherever a live Tally or a second block earns the width: the post-vote Tally (ballot left, live bars right); any poll surface with a visible live Tally beside the vote form (form left, Tally right); the standalone Results route (question and context left, Tally right); the landing page (statement and build notes left, Demo Poll right, the Demo Poll splitting its own form and Tally inside its column, `landing-footer` full-width below the grid); and the creator surface (Poll list left, selected Poll detail right). The availability grid becomes a true Voters × slots matrix. Wide shells cap at 1280px; everything else stays centered and single-column — a wider viewport buys air, not density. |
...
The moderation surface is one centered column at every viewport. On a phone the lookup field and `FIND POLL` stack; from the small breakpoint upward they share one row, while the target panel remains question → wrapping canonical link → two compact fact columns → Discovery action → Comment list. Long allowed references and Comment text wrap inside the measure, controls stay at least 44px high, and neither the 375px nor desktop silhouette permits horizontal overflow. The operator page uses the same light/dark tokens and adds no new token contract.
...
if necessary rather than creating horizontal overflow. The `landing-footer` is
distinct from the Poll footer: it is landing-page chrome, spans the full shell
width rather than the Poll measure, and appears on `/` only.
...
- **Moderation focus follows the result.** Initial load leaves document reading order undisturbed. A valid lookup focuses the target question; a safe lookup or mutation error focuses its alert; the canonical Discovery post-action redirect focuses the freshly verified success line. Document order remains lookup field → `FIND POLL` → outcome/target → canonical link → `DELIST` or `CLEAR DELISTED` → Comment delete controls. Comment confirmation starts on Cancel and restores its invoker on every enhanced close. Every control has a visible tokenized focus outline and at least a 44px target, and the native disclosure/form remains complete when JavaScript is unavailable.
...
2. The landing page opens with one Newsreader statement of what the platform is, then a short technical account of how it's built — Workers, D1, R2, Turnstile, Better Auth — in the same monospaced instrument voice as the rest of the product, ending at "The code is public." Non-technical readers get the first paragraph; technical readers get the second, and the repository link lives in the `landing-footer` (FR-25).
