...
| Creator — Export | `/creator/polls/:pollId/export.csv` (shipped); `.xlsx` planned | Poll detail | Owner-authorized raw Votes, Tally, and Summary download (FR-22) | UJ-1 | 1 |
...
| Export | Creator — Poll detail | One plain `EXPORT CSV` direct-download control with no configuration dialog or navigation. A sibling XLSX control joins when Story 4.4 ships. Available on the creator surface only (FR-22). |
