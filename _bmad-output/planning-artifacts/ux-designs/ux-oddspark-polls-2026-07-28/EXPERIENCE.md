...
| Creator — Export | `/creator/polls/:pollId/export.csv` (shipped); `.xlsx` planned | Poll detail | Owner-authorized raw Votes, Tally, and Summary download; XLSX is a direct synchronous download through 1,000 accepted Votes, with CSV as the larger-Poll path (FR-22) | UJ-1 | 1 |
...
| Export | Creator — Poll detail | One plain `EXPORT CSV` direct-download control with no configuration dialog or navigation. A sibling `EXPORT XLSX` control joins when Story 4.4 ships and starts a synchronous download for Polls with at most 1,000 accepted Votes. For 1,001 or more, activation yields no attachment and the exact plain-text line `XLSX export supports up to 1,000 accepted votes. Download CSV for larger Polls.` with HTTP `409`; CSV remains available. No partial or truncated workbook is ever presented. Available on the creator surface only (FR-22). |
