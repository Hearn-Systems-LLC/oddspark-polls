...
| `{breakpoints.lg}` and up | Two columns wherever a live Tally or a second block earns the width: the post-vote Tally (ballot left, live bars right); any poll surface with a visible live Tally beside the vote form (form left, Tally right); the standalone Results route (question and context left, Tally right); the landing page (statement and build notes left, Demo Poll right, the Demo Poll splitting its own form and Tally inside its column); and the creator surface (Poll list left, selected Poll detail right). The availability grid becomes a true Voters × slots matrix. Wide shells cap at 1280px; everything else stays centered and single-column — a wider viewport buys air, not density. |
