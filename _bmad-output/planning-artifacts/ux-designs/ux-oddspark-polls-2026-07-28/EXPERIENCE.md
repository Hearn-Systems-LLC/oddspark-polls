...
| `overlay` — Overlay | Delete confirmations, Voter Code panel | Three exist and none stack. Focus moves into the panel on open and is trapped there; `Esc` always closes; the scrim is clickable to dismiss on the two confirmations and **not** on the Voter Code panel, where a stray tap would lose an ungenerated list. On close by any means, **focus returns to the control that opened it**. The page behind does not scroll while an overlay is open. |
...
| Landing page: Demo Poll live | Landing page | The pinned Demo Poll renders as a complete, votable Poll inline — the same `poll-option` rows, the same vote button, the same `results-bar` group. Not a screenshot and not a reduced version. |
| Landing page: Demo Poll already voted | Landing page | The already-voted rejection renders inline with the Tally beneath it, exactly as it would on `/{link}`. A returning visitor sees the live bars rather than a dead form — which is the better demo anyway. |
...
- **Focus order** follows reading order on every surface: question → options → Comment → challenge → vote. Focus is never trapped except inside the two confirmations and the code panel, `Esc` always leaves, and **focus returns to the invoking control** when any of them closes, by any means.
