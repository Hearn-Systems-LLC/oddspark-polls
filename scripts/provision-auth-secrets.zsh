...
# replacement would invalidate sessions and orphan encrypted OAuth tokens.
...
if (( $# != 2 )); then
  print -u2 "Usage: $0 local|staging|production initialize|rotate-providers"
  exit 2
fi
...
if [[ "$operation" == "initialize" ]]; then
  if [[ "$target" != "local" ]]; then
    ...
    print -u2 "Bootstrap BETTER_AUTH_SECRET and BETTER_AUTH_URL in the target Worker dashboard, then run rotate-providers."
    exit 1
  fi
...
if [[ "$target" == "local" ]]; then
  ...
  if ! has_nonempty_local_binding BETTER_AUTH_SECRET ||
    ...
    print -u2 "Local provider rotation requires nonempty BETTER_AUTH_SECRET and BETTER_AUTH_URL bindings."
    exit 1
  fi
else
  existing_secret_names="$(remote_secret_names)"
  if [[ "$existing_secret_names" != *'"BETTER_AUTH_SECRET"'* ||
        "$existing_secret_names" != *'"BETTER_AUTH_URL"'* ]]; then
    print -u2 "$target auth is not fully initialized; bootstrap its master secret and base URL in the Worker dashboard first."
    exit 1
  fi
fi
...
if [[ "$target" == "local" ]]; then
  ...
  mv -f -- "$temporary_file" "$destination"
  trap - EXIT HUP INT TERM
  print "Rotated local provider credentials without changing BETTER_AUTH_SECRET or BETTER_AUTH_URL."
  exit 0
fi

cd "$project_root"
emit_provider_bindings | wrangler secret bulk --env "$target"
print "Rotated $target provider credentials without changing BETTER_AUTH_SECRET or BETTER_AUTH_URL."
